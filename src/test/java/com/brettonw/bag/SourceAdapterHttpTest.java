...
public class SourceAdapterHttpTest {
    ...
    public void testSourceAdapterHttpGet () {
        try {
            SourceAdapter sourceAdapter = new SourceAdapterHttp ("http://localhost:8080/bag-test-server/api?event=ip");
    ...
    public void testSourceAdapterHttpPost () {
        try {
            BagObject bagObject = new BagObject ()
                    ...
                    .put ("Last Name", "Wade");
            SourceAdapter sourceAdapter = new SourceAdapterHttp ("http://localhost:8080/bag-test-server/api?event=post-data", bagObject, MimeType.JSON);
            BagObject responseBagObject = BagObjectFrom.string (sourceAdapter.getStringData (), sourceAdapter.getMimeType ());
